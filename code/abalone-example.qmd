...
H~0~: **There is no effect of pH on abalone shell growth.**

H~A~: **There is an effect of pH on abalone shell growth.**
...
# Creating a new object for the cleaned data set 
abalone_clean <- abalone |> 
  # cleaning the column names within the dataset 
  clean_names() |> 
  # selecting specific columns, mean ph and change in area
  select(mean_p_h, change_in_area_mm2_d_1_25)  |> 
  # renaming the selected columns to be simpler
...
# Loading in ggplot as the base layer for the visualization
ggplot(data = abalone_clean, # 
       aes(x = mean_ph, # naming the axes on the visualization
           y = change_in_area)) +
  # Adding first layer as points to represent observations 
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
**The missing data point comes from row 1 where there is no data for the mean pH. A pair of tanks had a malfunctioning pH sensor**
...
abalone_model <- lm(
  change_in_area ~ mean_ph, # forumla: change in shell SA as a function of pH
...
**The column names are 'x', 'predicted', 'std.error', 'conf.low', 'conf.high', 'group' (not going to worry about 'group' column for now ) **
...
**This is a ggeffects object but it is also a data frame.**
...
**At pH = 8, the model predicts abalone shell growth at 5.36 [95% CI: 5.08, 5.64] mm^2 d^1**
